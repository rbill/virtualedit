 ...
 */
public class ResourceListMulti extends ViewPart implements ISelectionListener, IPartListener2
{
	MultiResourceSetInfo info;
	//ListViewer viewer;
	TableViewer viewer;
	...
	public void init(IViewSite site,IMemento memento) throws PartInitException {
		...
		for (IEditorReference part: wpage.getEditorReferences()) {
			...
			if (editor instanceof MultiXtextEditor) {
				setEditor((MultiXtextEditor)editor);
			}
		}
	}
	...
	public void createPartControl(Composite parent) {
		...
		viewer = tviewer;
		viewer.setContentProvider(new ResourceListMultiContentProvider());
	...
	public void createActions() {
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				ResourceListMulti.this.selectionChanged(newSelection);
			}
		});
	}
...
//		mgr.add(selectAllAction);
	...
	public void update(IWorkbenchPartReference ref) {
		if (ref instanceof IEditorReference) {
			...
			if (edpart instanceof MultiXtextEditor) {
				setEditor((MultiXtextEditor)edpart);
			}
		}
	}
	
	private void setEditor(MultiXtextEditor edpart) {
		editor = edpart;
		if (editor == null) {
			return;
		}
		MultiResourceSetInfo newInfo = editor.getResourceInfo();
			...
			if (update) {
				...
				if (editor == null || editor.getSuperDocument() == null) {
					...
					for (IEditorReference part: wpage.getEditorReferences()) {
						IEditorPart editor = part.getEditor(false);
						if (editor instanceof MultiXtextEditor) {
							MultiXtextEditor vme = (MultiXtextEditor)editor;
							setEditor(vme);
